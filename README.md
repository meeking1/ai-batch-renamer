...
- 按文件名、修改时间、大小排序
- 在资源管理器中定位选中文件
- 复制选中文件的原文件名主体，不包含扩展名，支持 Ctrl+C
- 复制预览到剪贴板
- 自然语言规则预览，本地启发式实现
- 多名称逐行替换
- 查找替换
- 模板命名
- 勾选后可将文件名中的汉字替换为随机数字、字母或数字+字母
- DeepSeek AI 自然语言命名
- 重命名前校验非法字符、重复名称、目标冲突、路径过长
- 拦截 Windows 保留设备名，例如 `CON`、`PRN`、`AUX`、`NUL`、`COM1`、`LPT1`
- 支持大小写-only 重命名、文件名互换和链式批量重命名
- 执行前确认
- 导出重命名预览 CSV
- JSON 操作日志
- 打开操作日志目录
...
仓库的 [Releases 页面](https://github.com/meeking1/ai-batch-renamer/releases) 会提供可下载版本。

每次推送 `v*` 标签，例如 `v0.1.0`，GitHub Actions 会自动构建并发布：

- `AiBatchRenamer-Setup-<version>.exe`：Windows 安装包
- `AiBatchRenamer-<version>.exe`：单文件版，双击直接运行
...
如需把文件名中的汉字替换掉，勾选“汉字替换为随机字符”，选择“随机数字 / 随机字母 / 随机数字+字母”，再点击“生成预览”。确认预览无误后点击“确认重命名”。
...
执行真实重命名时，程序会先把待处理文件移动到同目录临时文件，再移动到目标文件名。这样可以安全支持 `a.txt` 与 `b.txt` 互换名称、`a.txt -> b.txt -> c.txt` 这类链式重命名，以及仅修改大小写的重命名。
